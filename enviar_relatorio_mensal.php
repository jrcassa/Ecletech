...
<?php
/**
 * Script para FORÇAR envio de relatório MENSAL AGORA
 * Envia relatório do mês anterior completo para todos colaboradores com configuração ativa
 *
 * Uso: php enviar_relatorio_mensal.php
 */
...
echo "=== ENVIO FORÇADO - RELATÓRIO MENSAL ===\n\n";
...
try {
    ...
    $mesNome = $mesAnterior->format('F/Y');

    echo "📅 Período: {$periodoInicio} a {$periodoFim}\n";
    echo "📊 Tipo: mensal ({$mesNome})\n\n";
    ...
    if (empty($configs)) {
        echo "⚠️  Nenhuma configuração ativa encontrada para relatórios mensais.\n";
