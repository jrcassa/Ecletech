/**
 * Gerenciador de Transportadoras
 * Implementa CRUD completo de transportadoras com validação de permissões ACL
 */

const TransportadorasManager = {
    // Estado da aplicação
    state: {
        transportadoras: [],
        permissoes: {
    ...
    setupEventListeners() {
        ...
        this.elements.formTransportadora?.addEventListener('submit', (e) => this.salvar(e));
        ...
        try {
            ...
            if (permissoes) {
                this.state.permissoes = {
                    visualizar: permissoes.includes('transportadora.visualizar'),
                    criar: permissoes.includes('transportadora.criar'),
                    editar: permissoes.includes('transportadora.editar'),
                    deletar: permissoes.includes('transportadora.deletar')
                };
            }
    ...
    async carregarTransportadoras() {
        ...
        try {
            ...
            const response = await API.get(`/transportadora?${params.toString()}`);

            if (response.sucesso) {
                this.state.transportadoras = response.dados?.itens || [];
    ...
    renderizarTabela() {
        ...
        if (this.state.transportadoras.length === 0) {
        ...
        this.state.transportadoras.forEach(transportadora => {
            const tr = document.createElement('tr');

            const documento = transportadora.tipo_pessoa === 'PF' ?
                this.formatarCPF(transportadora.cpf) :
                this.formatarCNPJ(transportadora.cnpj);

            const ativo = transportadora.ativo == 1 || transportadora.ativo === true;

            tr.innerHTML = `
                <td>${transportadora.id}</td>
                <td>${this.escapeHtml(transportadora.nome)}</td>
                <td>${this.formatarTipoPessoa(transportadora.tipo_pessoa)}</td>
                <td>${documento || '-'}</td>
                <td>${this.escapeHtml(transportadora.email) || '-'}</td>
                <td>${this.formatarTelefone(transportadora.telefone) || '-'}</td>
                <td>
                ...
                <td>
                    <div class="actions">
                        ${this.state.permissoes.editar ?
                            `<button class="btn btn-small" onclick="TransportadorasManager.editar(${transportadora.id})">Editar</button>` :
                            ''}
                        ${this.state.permissoes.deletar ?
                            `<button class="btn btn-small btn-danger" onclick="TransportadorasManager.deletar(${transportadora.id})">Deletar</button>` :
                            ''}
                    </div>
                </td>
            `;
    ...
    /**
     * Abre modal para novo transportadora
     */
    abrirModalNovo() {
        ...
        this.elements.modalTitle.textContent = 'Novo Transportadora';
        this.elements.formTransportadora.reset();
    ...
    async editar(id) {
        try {
            const response = await API.get(`/transportadora/${id}`);

            if (response.sucesso && response.dados) {
                const transportadora = response.dados;
                this.state.editandoId = id;

                this.elements.modalTitle.textContent = 'Editar Transportadora';

                // Dados básicos
                document.getElementById('transportadoraId').value = transportadora.id;
                document.getElementById('tipoPessoa').value = transportadora.tipo_pessoa || 'PF';
                document.getElementById('nome').value = transportadora.nome || '';
                document.getElementById('ativo').value = transportadora.ativo == 1 ? '1' : '0';

                // Campos específicos
                if (transportadora.tipo_pessoa === 'PF') {
                    document.getElementById('cpf').value = this.formatarCPF(transportadora.cpf) || '';
                    document.getElementById('rg').value = transportadora.rg || '';
                    document.getElementById('dataNascimento').value = transportadora.data_nascimento || '';
                } else {
                    document.getElementById('cnpj').value = this.formatarCNPJ(transportadora.cnpj) || '';
                    document.getElementById('razaoSocial').value = transportadora.razao_social || '';
                    document.getElementById('inscricaoEstadual').value = transportadora.inscricao_estadual || '';
                    document.getElementById('inscricaoMunicipal').value = transportadora.inscricao_municipal || '';
                    document.getElementById('tipoContribuinte').value = transportadora.tipo_contribuinte || '';
                }

                document.getElementById('email').value = transportadora.email || '';
                document.getElementById('telefone').value = this.formatarTelefone(transportadora.telefone) || '';
                document.getElementById('celular').value = this.formatarTelefone(transportadora.celular) || '';
                document.getElementById('observacoes').value = transportadora.observacoes || '';

                // Controla campos condicionais
                this.controlarCamposCondicionais();

                // Carrega contatos
                this.state.contatos = transportadora.contatos || [];
                this.renderizarContatos();

                // Carrega endereços
                this.state.enderecos = transportadora.enderecos || [];
            ...
            const mensagemErro = error.data ?
                Utils.Errors.formatarMensagem(error.data) :
                'Erro ao carregar transportadora';
    ...
    async salvar(e) {
        ...
        try {
            ...
            if (this.state.editandoId) {
                // Atualizar
                response = await API.put(`/transportadora/${this.state.editandoId}`, dados);
            } else {
                // Criar
                response = await API.post('/transportadora', dados);
            }

            if (response.sucesso) {
                this.fecharModal();
                this.carregarTransportadoras();
                Utils.Notificacao.sucesso(response.mensagem || 'Transportadora salvo com sucesso!');
            }
        } catch (error) {
            // Exibe mensagem de erro com detalhes de validação
            this.showModalError(error.data || 'Erro ao salvar transportadora');
            Utils.Notificacao.erro(error.data || 'Erro ao salvar transportadora');
     ...
     */
    async deletar(id) {
        if (!confirm('Tem certeza que deseja deletar este transportadora?')) {
            return;
        }

        try {
            const response = await API.delete(`/transportadora/${id}`);

            if (response.sucesso) {
                this.carregarTransportadoras();
                Utils.Notificacao.sucesso(response.mensagem || 'Transportadora deletado com sucesso!');
            }
        } catch (error) {
            // Usa Utils para formatar mensagem de erro
            const mensagemErro = error.data ?
                Utils.Errors.formatarMensagem(error.data) :
                'Erro ao deletar transportadora';
        ...
        this.state.contatos.forEach((contato, index) => {
            ...
            div.innerHTML = `
                ...
                <button type="button"
                        ...
                        onclick="TransportadorasManager.removerContato(${index})">
    ...
    renderizarEnderecos() {
        ...
        this.state.enderecos.forEach((endereco, index) => {
            ...
            div.innerHTML = `
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text"
                               ...
                               onblur="TransportadorasManager.buscarCEP(this.value, ${index})">
                    </div>
                    <div class="form-group">
                ...
                <button type="button"
                        class="btn btn-small btn-danger"
                        onclick="TransportadorasManager.removerEndereco(${index})">
                    <i class="fas fa-trash"></i> Remover
                </button>
            `;
...
document.addEventListener('DOMContentLoaded', () => {
    TransportadorasManager.init();
});

// Expõe globalmente para uso nos event handlers inline
window.TransportadorasManager = TransportadorasManager;
