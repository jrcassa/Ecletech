...
const TiposEnderecosManager = {
    ...
    renderizarTabela() {
        ...
        this.state.tipos.forEach(tipo => {
            ...
            tr.innerHTML = `
                <td>${tipo.id}</td>
                <td>${this.escapeHtml(tipo.id_externo || '-')}</td>
    ...
    async editar(id) {
        try {
            ...
            if (response.sucesso && response.dados) {
                ...
                document.getElementById('id_externo').value = tipo.id_externo || '';
    ...
    async salvar(e) {
        ...
        const idExterno = document.getElementById('id_externo').value.trim();
        if (idExterno) {
            dados.id_externo = idExterno;
        }
