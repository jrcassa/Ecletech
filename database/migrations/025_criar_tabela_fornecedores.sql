...
-- Descrição: Tabela para gerenciamento de fornecedores (PF e PJ)
-- Data: 2025-11-11
-- =====================================================
...
CREATE TABLE IF NOT EXISTS `fornecedores` (
  ...
  `ativo` BOOLEAN NOT NULL DEFAULT TRUE COMMENT 'Fornecedor ativo/inativo',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Cadastro de fornecedores (PF e PJ)';

-- =====================================================
-- Inserir permissões para o módulo de fornecedores
-- =====================================================

INSERT INTO `colaborador_permissions` (`nome`, `codigo`, `descricao`, `modulo`, `ativo`, `criado_em`) VALUES
('Visualizar Fornecedores', 'fornecedor.visualizar', 'Permite visualizar a lista de fornecedores e detalhes', 'fornecedor', 1, NOW()),
('Criar Fornecedores', 'fornecedor.criar', 'Permite cadastrar novos fornecedores', 'fornecedor', 1, NOW()),
('Editar Fornecedores', 'fornecedor.editar', 'Permite editar informações dos fornecedores', 'fornecedor', 1, NOW()),
('Deletar Fornecedores', 'fornecedor.deletar', 'Permite remover fornecedores (soft delete)', 'fornecedor', 1, NOW());
...
SET @perm_visualizar = (SELECT id FROM colaborador_permissions WHERE codigo = 'fornecedor.visualizar');
SET @perm_criar = (SELECT id FROM colaborador_permissions WHERE codigo = 'fornecedor.criar');
SET @perm_editar = (SELECT id FROM colaborador_permissions WHERE codigo = 'fornecedor.editar');
SET @perm_deletar = (SELECT id FROM colaborador_permissions WHERE codigo = 'fornecedor.deletar');
